...
async function calendarInit() {
  ...
  const events = agenda.List.map((item) => ({
    ...item,
    image: item?.image?.asset?._ref ? makeImgUrl(item?.image?.asset?._ref) : "",
  }));
  ...
  var calendar = new FullCalendar.Calendar(calendarEl, {
    ...
    eventClick: function (info) {
      ...
      const content = `
          ...
          ${
            props?.image
              ? `<img src="${props.image}" alt="${info.event.title}" style="max-width: 100%; height: auto;" />`
              : ""
          }
          ...
          ${
            props?.price
              ? `<p><strong>Precio de entrada:</strong> $${props.price}</p>`
              : ""
          }
          ${
            info.event.start
